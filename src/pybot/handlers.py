...
class TelegramCommandHandler:
    def __init__(self, redis_repo: RedisRepository, chatgpt_service: ChatGPTService,
                 user_service: UserService, event_service: EventService):
    ...
    def register(self, update: Update, context: CallbackContext[Any, Any, Any]) -> None:
        """Register user interests with optional description: /register gaming vr "description" """
        ...
        # Split args into interests and description
        args = context.args
        interests = []
        description = ""
    ...
    def events(self, update: Update, context: CallbackContext[Any, Any, Any]) -> None:
        username = update.message.from_user.username or str(update.message.from_user.id)
        interests = self.user_service.find_matches(username)
